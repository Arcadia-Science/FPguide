#!/usr/bin/env bash
# Gibbs-sampling design campaign job: pure ESM-2 masked-LM generation for the 24 scaffold->target
# pairs, one pair at a time (6 trials each, any-order masking, T=1 k=10, 3 iters). Per-position
# choice samples DIRECTLY from ESM-2's top-k masked-LM conditional (raw log-probs, T=1) -- no
# surrogate peak-error (MAE) term. The surrogate is loaded only to record (ex, em)/peak_err as a
# diagnostic.
...
# Tier-B run: endpoint-only ppl during the design cycle for speed, then backfill the blank
# intermediate-round ppl cells in a single deduped pass afterwards (see design_campaign.py).
PPL="${PPL:-endpoints}"
