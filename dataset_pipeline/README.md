...
`build_dataset.py`, `build_spectra_dataset.py`, and `make_dual_split.py` run on CPU in seconds — no ESM-2 or GPU needed.
...
The notebook was originally emitted by a `_build_visualize_curation.py` generator script, now retired to
`archive/` — the `.ipynb` is the only source, and has carried hand edits the generator never had. Edit it
directly.

## What the refactor changed

This pipeline replaced an earlier notebook-based data-processing stage that lived in a `peak_design/`
folder, since dissolved. Peak came out **unchanged at 758 rows**: ecliptic pHluorin is now kept (its peak
is unambiguous) and iFP2.0 is now dropped (biliverdin). The brightness/pKa deltas against their old
outputs are in [Archived: brightness & pKa](#archived-brightness--pka) above. The script that diffed the
two eras is retired to `archive/` along with the era it compared against; each archived `curate_meta.json`
preserves the numbers.

Two pieces of that old wiring survive here and are described above rather than in this section:
`make_dual_split.py` still writes `data/peak/curated/dual_splits.csv` with the original logic (seed 0,
70/15/15, coordinated so `S_test ⊆ O_train` and `O_test ⊆ S_train`), and `embed.py` still builds the peak
ESM-2 cache (see [Usage](#usage)). Everything downstream of them — the learning-curve and
surrogate/oracle notebooks of that era — now lives under the repo's `archive/` trees.
...
Rules and thresholds here encode specific biological judgment calls (sensor handling, on-state policy, the NN cutoff). If you change one, rebuild into a scratch directory and diff the result against the committed one — the per-stage counts and exclusion reasons in `curate_meta.json` are the readable record, and every delta should be explainable in terms of the principle at the top of this file:

```bash
python build_dataset.py --target peak --outdir /tmp/peak_check
diff /tmp/peak_check/curate_meta.json data/peak/curated/curate_meta.json
```

`created` and `source_proteins` always differ (build date, and a path stored relative to `--outdir`);
anything else in that diff is a real change in what the rules keep. The stage counts the build prints
are the same numbers, if you'd rather read them off stdout.
